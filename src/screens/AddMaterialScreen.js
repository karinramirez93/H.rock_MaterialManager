...
const UNIT_OPTIONS = ['Unit', 'Box', 'Bundle', 'Reel', 'Length (ft)', 'Length (in)', 'Bottle', 'Custom Unit'];
...
export default function AddMaterialScreen({ navigation, currentUser }) {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const [catalog, fetchedCategories] = await Promise.all([
    ...
    <SafeAreaView style={styles.container} edges={['left', 'right', 'bottom']}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={90}>
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          ...
          {allowedUnits.includes('Custom Unit') ? (
            <View style={styles.newCategoryBox}>
...
const styles = StyleSheet.create({
  ...
  optionButton: { backgroundColor: '#112240', paddingVertical: 10, borderRadius: 8, borderWidth: 1, borderColor: '#233554', marginBottom: 10, width: '31%', alignItems: 'center' },
  optionButtonActive: { backgroundColor: '#0275d8', borderColor: '#64ffda' },
  optionText: { color: '#8892b0', fontWeight: 'bold', fontSize: 11 },
